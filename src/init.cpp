...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, wire, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-backupzwire=<n>", strprintf(_("Enable automatic wallet backups triggered after each zWire minting (0-1, default: %u)"), 1));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Wire Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZWireBackups = GetBoolArg("-backupzwire", true);
        pwalletMain->setZWireAutoBackups(fEnableZWireBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeWireAmount = GetArg("-anonymizewireamount", 0);
//    if (nAnonymizeWireAmount > 999999) nAnonymizeWireAmount = 999999;
//    if (nAnonymizeWireAmount < 2) nAnonymizeWireAmount = 2;
