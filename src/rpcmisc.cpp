...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zwireObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zwireObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zWIREsupply", zwireObj));
...
Value getstakingstatus(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("enoughbalance", nBalance * COIN > Params().MinStakeInput()));
    }
